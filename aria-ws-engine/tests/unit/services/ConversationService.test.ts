...
describe('ConversationService', () => {
  ...
  describe('error handling', () => {
    it('propagates LLM errors from processUserInput', async () => {
      const failingLLM = createMockLLMConnector();
      failingLLM.complete.mockRejectedValue(new Error('LLM service unavailable'));

      const deps = buildDeps({ llmConnector: failingLLM });
      const service = new ConversationService(deps);

      await expect(service.processUserInput(session._id, 'Hello')).rejects.toThrow(
        'LLM service unavailable'
      );
    });

    it('propagates LLM errors from initialize', async () => {
      const failingLLM = createMockLLMConnector();
      failingLLM.complete.mockRejectedValue(new Error('Rate limit exceeded'));

      const deps = buildDeps({ llmConnector: failingLLM });
      const service = new ConversationService(deps);

      await expect(service.initialize(session._id)).rejects.toThrow('Rate limit exceeded');
    });

    it('propagates error when session is not found', async () => {
      const deps = buildDeps({
        sessionRepo: {
          findByIdOrThrow: jest.fn().mockRejectedValue(new Error('Session not found')),
        },
      });
    ...
    it('propagates error when agent is not found', async () => {
      const deps = buildDeps({
        agentRepo: {
          findByIdOrThrow: jest.fn().mockRejectedValue(new Error('Agent not found')),
        },
      });
    ...
    it('stores user message before LLM failure', async () => {
      const failingLLM = createMockLLMConnector();
      failingLLM.complete.mockRejectedValue(new Error('LLM timeout'));

      const deps = buildDeps({ llmConnector: failingLLM });
      const service = new ConversationService(deps);

      await expect(service.processUserInput(session._id, 'Hello', 100)).rejects.toThrow();

      // User message should have been stored before LLM call
      expect(deps.conversationRepo.addMessage).toHaveBeenCalledWith(
        session._id,
        expect.objectContaining({ role: 'user', content: 'Hello' })
      );
      // Assistant message should NOT have been stored
      expect(deps.conversationRepo.addMessage).not.toHaveBeenCalledWith(
        session._id,
  ...
  describe('session resume edge cases', () => {
    it('generates new response when last message is from user on resume', async () => {
      const deps = buildDeps();
      ...
      const existingConversation = createMockConversation({
        ...
        messages: [
          createMockMessage({ role: 'system', content: 'context' }),
          createMockMessage({ role: 'user', content: 'I was saying...' }),
        ],
      });
      ...
      expect(result.isNewSession).toBe(false);
      // Should call LLM to recover from interrupted state
      expect(deps.llmConnector.complete).toHaveBeenCalled();
      expect(result.text).toBe('Hello there!');
    });

    it('returns empty text when conversation has no messages on resume', async () => {
      const deps = buildDeps();
  ...
  describe('concurrency (KeyedMutex)', () => {
    it('serializes concurrent processUserInput calls on the same session', async () => {
      const callOrder: string[] = [];

      const slowLLM = createMockLLMConnector({ text: 'Response', action: undefined });
      slowLLM.complete.mockImplementation(
        () =>
          new Promise((resolve) =>
            setTimeout(() => {
              callOrder.push('llm-complete');
              resolve({
                response: { text: 'Response', action: undefined },
      ...
      const deps = buildDeps({ llmConnector: slowLLM });
      const service = new ConversationService(deps);

      // Fire two concurrent calls on the same session
      const [result1, result2] = await Promise.all([
        service.processUserInput(session._id, 'First'),
        service.processUserInput(session._id, 'Second'),
      ]);
      ...
      // LLM should have been called exactly twice, sequentially
      expect(slowLLM.complete).toHaveBeenCalledTimes(2);
      expect(callOrder).toEqual(['llm-complete', 'llm-complete']);
    });

    it('runs operations on different sessions concurrently', async () => {
      ...
      const deps = buildDeps({
        ...
        sessionRepo: {
          findByIdOrThrow: jest.fn().mockImplementation((id: ObjectId) => {
            if (id.equals(session._id)) return Promise.resolve(session);
      ...
      await Promise.all([
        service.processUserInput(session._id, 'Session 1'),
        service.processUserInput(session2._id, 'Session 2'),
      ]);
      const elapsed = Date.now() - start;

      // Both completions should have happened (different keys = parallel)
      expect(completionOrder).toHaveLength(2);
      // Parallel execution should be roughly 50ms, not 100ms
      // Use a generous threshold to avoid flaky tests
      expect(elapsed).toBeLessThan(150);
    });

    it('releases mutex even when operation throws', async () => {
      const failingLLM = createMockLLMConnector();
      failingLLM.complete
        .mockRejectedValueOnce(new Error('First call fails'))
        .mockResolvedValueOnce({
          response: { text: 'Recovered', action: undefined },
          usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
          finishReason: 'stop',
        });
      ...
      // First call fails
      await expect(service.processUserInput(session._id, 'Fail')).rejects.toThrow(
        'First call fails'
      );

      // Second call should NOT be deadlocked — mutex must have been released
      const result = await service.processUserInput(session._id, 'Retry');
  ...
  describe('action error handling', () => {
    it('returns actionError when action execution fails', async () => {
      const actionLLM = createMockLLMConnector({
        text: 'Moving on!',
        action: 'STEP_COMPLETED',
      });
      ...
      // Make the session repo's updateCurrentStep fail (used by STEP_COMPLETED action)
      deps.sessionRepo.updateCurrentStep.mockRejectedValue(new Error('DB write failed'));

      const service = new ConversationService(deps);
      const result = await service.processUserInput(session._id, 'Next please');

      // Should still return text, but with actionError
