...
import { SessionDocument, SessionStatus } from '@models';
...
export class SessionRepository extends BaseRepository<SessionDocument> {
  ...
  async markComplete(sessionId: string): Promise<SessionDocument | null> {
    const session = await this.updateStatus(new ObjectId(sessionId), 'completed');
