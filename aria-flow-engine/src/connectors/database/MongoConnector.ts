...
import { DatabaseError, ConfigurationError } from "@utils";
...
export class MongoConnector extends EventEmitter implements IDatabase {
  ...
  private setupEventListeners(): void {
    ...
    this.client.on("serverHeartbeatFailed", () => {
      console.warn("[MongoDB] Server heartbeat failed");
    });
  }
...
export function getMongoConnector(config?: DatabaseConfig): MongoConnector {
  if (!instance) {
    if (!config) {
      throw new ConfigurationError("Config required for first initialization");
    }
